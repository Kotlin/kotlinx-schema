...
object SpecCoverageReportGenerator {
    ...
    private fun StringBuilder.appendOverallStatistics(stats: OverallStats) {
        appendLine("# 📊 JSON Schema Draft 2020-12 Spec Coverage Report")
        appendLine()
        appendLine("Generated by conformance tests for `JsonSchemaDefinition`.")
        appendLine()
        ...
        appendLine("- **✅ Successfully Parsed**: ${stats.totalSuccess}")
        appendLine("- **❌ Failed to Parse**: ${stats.totalFailures}")
    ...
    private fun StringBuilder.appendResultsByFile(fileStats: Map<String, FileStats>) {
        appendLine("## 📁 Results by Test File")
        appendLine()
        appendLine("| File | Success Rate | ✅ Passed | ❌ Failed |")
        appendLine("|------|--------------|----------|----------|")
    ...
    private fun StringBuilder.appendTopFailureTypes(failures: List<SchemaTestResult>) {
        appendLine("### 🏷️ Top Failure Types")
        appendLine()
    ...
    private fun StringBuilder.appendFailureItem(failure: SchemaTestResult) {
        ...
        appendLine("#### ❌ **${failure.testSuiteDescription}** [$stage]")
        appendLine()
